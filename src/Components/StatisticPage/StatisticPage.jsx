...
const renderCirclePacking = () => {
  ...
  const circle = g
      ...
      .attr("fill", (d) => (d.children ? "#1E90FF" : "#32CD32"))
      .attr("stroke", "#000")
...
const renderTree = () => {
  ...
  const zoom = d3.zoom()
    .scaleExtent([1, 5]) // Limites de zoom (min, max)
    .translateExtent([[-width, -height], [width, height]]) // Limites de déplacement
    .on("zoom", (event) => {
  ...
  const node = g
    ...
    .attr(
      "transform",
      (d) => `rotate(${(d.x * 180) / Math.PI - 90}) translate(${d.y + 30},0)` // Ajout de 30 pour espacer les nœuds
    );
